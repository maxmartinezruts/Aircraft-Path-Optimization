...
def DP_swap_fuel(pl_seq,fuel_positions,pos,try_sequence):
   ...
   # print(sequence, pl_seq)
    ...
    if pos == 0:
        ...
        fuel_position = get_fuel(try_sequence)
...
class Plane:
    ...
    def next_point(self, pos, path):
        ...
        for p in path:
            pygame.draw.circle(screen, yellow, cartesian_to_screen(p), 10)
        pygame.display.flip()
        #time.sleep(0.02)
            ...
            if button_heatmap.collidepoint(mouse_pos):
                ...
                if button_draw.collidepoint(mouse_pos):
                    ...
                    if drawing:
                        ...
                        for edge1 in edges:
                            ...
                            for edge2 in edges:
                                if edge1['pol'] != edge2['pol']:
                                    ...
                                    if intersect(Point(edge1['pts'][0]), Point(edge1['pts'][1]), Point(edge2['pts'][0]),
                                                 Point(edge2['pts'][1])):
                                        intersection = True
                            if not intersection:
                                graph.add_edge(edge1['nodes'][0], edge1['nodes'][1],
                                               np.linalg.norm(edge1['pts'][0] - edge1['pts'][1]))
                        print(edges)
                        drawing_pol = []
                        tangents = []
                        ...
                        print(graph.distances, 'edges')
                        print(dijsktra(graph, 1))
                        table = dijsktra(graph, 1)
                        nxt = 3
                        path = [nxt]
                        while nxt != 1:
                            nxt = table[1][nxt]
