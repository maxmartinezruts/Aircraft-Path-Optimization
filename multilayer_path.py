...
def set_edges():
    ...
    for node in graph.nodes:
        for linked_node in graph.nodes:
            ...
            if linked_node != node and w_distance<1:
...
while True:
    ...
    graph = Graph()
    fibsphere = fibonacci_sphere(300)
    for point in fibsphere:
        size = int(len(graph.nodes))
        graph.add_node(size, point)
        pygame.draw.circle(screen, red, cartesian_to_screen(point), 1)
    pygame.display.flip()
    time.sleep(1)
    ...
    for id in graph.nodes:
        pygame.draw.circle(screen, red, cartesian_to_screen(graph.ids[id]), int(weights.total[id])+1)
    ...
    for i in range(4):
        ...
        while nxt != center_node:
            ...
            closenodes.append(nxt)
        for id in graph.nodes:
            pygame.draw.circle(screen, red, cartesian_to_screen(graph.ids[id]), int(weights.total[id]))
        ...
        for node1 in closenodes:
            for node2 in closenodes:
                p1 =graph.ids[node1]
                p2 =graph.ids[node2]
                if node1 != node2  and np.linalg.norm(graph.ids[node1]-graph.ids[node2])/distance<math.sqrt(2)+0.01:
        ...
        for point in allpoints:
                ...
                graph.add_node(size, point)
        print(len(graph.nodes),len(graph.distances))
        weights = Weights(['density'])
        weights.set_total()
        distance /=2
        set_edges()
